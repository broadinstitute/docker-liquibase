...
CHGLOG_DIR=${CHGLOG_DIR:-"/changelog"}
...
then
  ...
  vault read --field=keystore "secret/dsde/${APP_PROJ}/${ENV}/${DB_NAME}/${DB_NAME}-mysql" | base64 -d > /tmp/keystore
  vault read --field=truststore "secret/dsde/${APP_PROJ}/${ENV}/${DB_NAME}/${DB_NAME}-mysql" | base64 -d > /tmp/truststore
...
# DB_PASS=`vault read --field=admin_password "secret/dsde/${APP_PROJ}/${ENV}/${DB_NAME}/secrets"`

echo "Running liquibase using the following command: "
...
liquibase  --driver=com.mysql.jdbc.Driver ${LOG_OPTS} --changeLogFile=${CHGLOG_DIR}/changelog.xml --url="${DB_URL}" --username=${DB_USER} --password=${DB_PASS}  migrate
